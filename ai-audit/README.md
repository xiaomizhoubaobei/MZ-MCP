...
>
> ✅ `alert_triage_example.yaml` 已按官方框架 `doc/GRAMMAR.md` 语法编写并针对本项目（**JavaScript/TypeScript**）适配：
...
└── personalities/
    ...
    └── js_ts_auditer.yaml           # 项目自带的 JS/TS 审计人格（personality）
```
...
**为什么需要自带的 JS/TS 人格？** 官方框架默认自带的安全审计人格是
`seclab_taskflow_agent.personalities.c_auditer`，它是针对 **C 语言**设计的
（系统提示词明确"Find vulnerabilities in any provided C code"）。
本项目是 **JavaScript/TypeScript** 仓库（`src/**`），为获得正确的审计结果，示例改用项目自带的
`ai_audit.personalities.js_ts_auditer`（面向 JS/TS / MCP Server 的审计人格），
...
#       ai_audit/personalities/js_ts_auditer.yaml
```
...
3. 按官方配置指南配好 LLM 模型 + GitHub PAT + MCP Server。
   ...
   > 同时，`js_ts_auditer.yaml` personality 的 `toolboxes` 也已加入
...
使其可通过 `ai_audit.personalities.js_ts_auditer` 解析，然后把
`alert_triage_example.yaml` 中的 agents 引用改为
`ai_audit.personalities.js_ts_auditer` 即可。
...
- **针对 JavaScript/TypeScript**：适配本仓库 `src/**` 的前端/Node.js 代码审计场景。
...
> 📌 本示例由 CNB NPC CodeBuddy 为 `XMZZUZHI/Github/MZ-MCP` 生成的测试用示例，
